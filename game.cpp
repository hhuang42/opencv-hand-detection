...
int player0_score = 0;
int player1_score = 0;
int player0_height = game_height/2;
int player1_height = game_height/2;
int paddle_semiheight = game_height/6;
int ball_width = 10; 
Point ball = Point(320, 240);
int y_vel = 0;
int x_dir = 0;
...
void game_logic(){
    ...
    if (ball.x == 110){
        if ((ball.y - player0_height <= paddle_semiheight) &&
            (player0_height - ball.y <= paddle_semiheight)){
                y_vel = (ball.y - player0_height)/4;
                x_dir = 1;
            }
    }
    if (ball.x == 530){
        if ((ball.y - player1_height <= paddle_semiheight) &&
            (player1_height - ball.y <= paddle_semiheight)){
                y_vel = (ball.y - player1_height)/4;
                x_dir = 0;
            }
    }
...
void display(ImageInfo *ctx)
{
	...
	if (true) {
        ...
        putText(image,std::to_string(player0_score), Point(40,80), 
        FONT_HERSHEY_SIMPLEX, 4, GREY, 5);
        putText(image,std::to_string(player1_score), Point(520,80), 
        FONT_HERSHEY_SIMPLEX, 4, GREY, 5);
#if defined(SHOW_HAND_CONTOUR)
		drawContours(image, contours, 0, BLUE);
#endif
		//circle(image, ctx->hand_center, 5, PURPLE, 1, CV_AA, 0);
		rectangle(image, Point(80,player0_height-paddle_semiheight),
		          Point(100, player0_height+paddle_semiheight),BLUE);
		          
		rectangle(image, Point(540,player1_height-paddle_semiheight),
		          Point(560, player1_height+paddle_semiheight),GREEN);
		rectangle(image, Point(ball.x-ball_width,ball.y-ball_width),
		          Point(ball.x+ball_width, ball.y + ball_width),GREY, -1);
